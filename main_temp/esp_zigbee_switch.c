...
static void send_report_attr_command()
{
...
{
    while (1) {
        int16_t measured_value = rand() % (3000 + 1 - 1000);;
...
static esp_zb_ep_list_t *custom_multistate_sensor_ep_create()
{
    esp_zb_attribute_list_t *esp_zb_multistate_cluster = esp_zb_zcl_attr_list_create(ESP_ZB_ZCL_CLUSTER_ID_MULTI_INPUT);
    esp_zb_multistate_value_cluster_add_attr(esp_zb_multistate_cluster, ESP_ZB_ZCL_ATTR_)
...
void app_main(void)
{
    ...
    xTaskCreate(esp_app_temp_sensor_handler, "SensorTast", 4096, NULL, 6, NULL);
